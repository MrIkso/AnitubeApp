...
public class CollectionsPagingSource extends RxPagingSource<Integer, CollectionModel> {
    ...
    public CollectionsPagingSource(@NonNull AnitubeApiService service, @NonNull CollectionsParser mapper) {
    ...
    @NotNull
    @Override
    public Single<LoadResult<Integer, CollectionModel>> loadSingle(@NotNull LoadParams<Integer> loadParams) {

        // If page number is already there then init page variable with it otherwise we are loading
        // fist page
        int page = loadParams.getKey() != null ? loadParams.getKey() : 1;

        // Log.d("CollectionsPagingSource", "loadSingle: loading page " + page);

        // Send request to server with page number
        return service.getCollections(page)
    ...
    private LoadResult<Integer, CollectionModel> toLoadResult(List<CollectionModel> data, int page, int maxPage) {
    ...
    public Integer getRefreshKey(@NotNull PagingState<Integer, CollectionModel> state) {
        ...
        LoadResult.Page<Integer, CollectionModel> anchorPage = state.closestPageToPosition(anchorPosition);
        if (anchorPage == null) {
            return null;
        }
