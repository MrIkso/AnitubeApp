package com.mrikso.player.dtpv.youtube;
...
import com.mrikso.player.dtpv.PlayerDoubleTapListener;
import com.mrikso.player.dtpv.SeekListener;
import com.mrikso.player.dtpv.youtube.views.CircleClipTapView;
import com.mrikso.player.dtpv.youtube.views.SecondsView;
...
public final class YouTubeOverlay extends ConstraintLayout implements PlayerDoubleTapListener {
    ...
    public YouTubeOverlay(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        ((CircleClipTapView) findViewById(R.id.circle_clip_tap_view)).setPerformAtEnd(new Runnable() {
            @Override
            public void run() {
    ...
    private void initializeAttributes() {
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.YouTubeOverlay, 0, 0);

            // PlayerView => see onAttachToWindow
            playerViewRef = a.getResourceId(R.styleable.YouTubeOverlay_yt_playerView, -1);

            // Durations
            setAnimationDuration((long) a.getInt(R.styleable.YouTubeOverlay_yt_animationDuration, 650));

            seekSeconds = a.getInt(R.styleable.YouTubeOverlay_yt_seekSeconds, 10);

            setIconAnimationDuration((long) a.getInt(R.styleable.YouTubeOverlay_yt_iconAnimationDuration, 750));
            ...
            this.setIcon(a.getResourceId(R.styleable.YouTubeOverlay_yt_icon, R.drawable.ic_play_triangle));
    ...
    protected void onAttachedToWindow() {
        ...
        if (playerViewRef != -1) playerView((DoubleTapPlayerView) ((View) getParent()).findViewById(playerViewRef));
    }
    ...
    */
    ...
    private void changeConstraints(boolean forward) {
        ...
        if (forward) {
            ...
            constraintSet.connect(secondsView.getId(), ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END);
        } else {
            constraintSet.clear(secondsView.getId(), ConstraintSet.END);
            constraintSet.connect(
                    secondsView.getId(), ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START);
        }
        // secondsView.start();
    ...
    public boolean isLocked() {
        return this.playerView.isLocked();
    }
}
