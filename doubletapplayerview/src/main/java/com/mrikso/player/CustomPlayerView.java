...
public class CustomPlayerView extends PlayerView
            ...
            case MotionEvent.ACTION_CANCEL:
                if (handleTouch) {
                    if (gestureOrientation == Orientation.HORIZONTAL) {
                        ...
                        postDelayed(
                                textClearRunnable, isHandledLongPress ? MESSAGE_TIMEOUT_LONG : MESSAGE_TIMEOUT_TOUCH);
                    }
    ...
    public boolean tap() {
        ...
        if (!isControllerFullyVisible()) {
            showController();
            return true;
        } else if (haveMedia && getPlayer() != null && getPlayer().isPlaying()) {
            hideController();
            return true;
        }
        return false;
    ...
    public boolean onScroll(MotionEvent motionEvent, MotionEvent motionEvent1, float distanceX, float distanceY) {
        ...
        if (motionEvent.getY() < IGNORE_BORDER
                ...
                || motionEvent.getY() > getHeight() - IGNORE_BORDER
                || motionEvent.getX() > getWidth() - IGNORE_BORDER) return false;
        ...
        if (gestureOrientation == Orientation.HORIZONTAL || gestureOrientation == Orientation.UNKNOWN) {
            ...
            if (Math.abs(gestureScrollX) > SCROLL_STEP
                    || (gestureOrientation == Orientation.HORIZONTAL && Math.abs(gestureScrollX) > SCROLL_STEP_SEEK)) {
                ...
                if (gestureOrientation == Orientation.UNKNOWN) {
                    ...
                    clearIcon();
                    seekLastPosition = seekStart = getPlayer().getCurrentPosition();
                    seekChange = 0L;
                    seekMax = getPlayer().getDuration();

                    if (!isControllerFullyVisible()) {
                        seekProgress = true;
                        // playerControlView.showProgress();
                    }
                }
                ...
                if (haveMedia) {
                    if (gestureScrollX > 0) {
                        if (seekStart + seekChange - SEEK_STEP * distanceDiff >= 0) {
                            ExoPlayer exo = (ExoPlayer) getPlayer();
                        ...
                        ExoPlayer exo = (ExoPlayer) getPlayer();
                        exo.setSeekParameters(SeekParameters.NEXT_SYNC);
                    ...
                    for (long start : chapterStarts) {
                        if ((seekLastPosition < start && position >= start)
                                || (seekLastPosition > start && position <= start)) {
    ...
    public void onLongPress(MotionEvent motionEvent) {
        if (locked || (getPlayer() != null && getPlayer().isPlaying())) {
            ...
            if (locked && isControllerFullyVisible()) {
                hideController();
            }
        }
    }
    ...
    public float getScaleFit() {
        return Math.min(
                (float) getHeight() / (float) getVideoSurfaceView().getHeight(),
    ...
    public void setIconVolume(boolean volumeActive) {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(
                volumeActive ? R.drawable.ic_volume_up_24dp : R.drawable.ic_volume_off_24dp, 0, 0, 0);
    }

    public void setHighlight(boolean active) {
        if (active) exoErrorMessage.getBackground().setTint(Color.RED);
        else exoErrorMessage.getBackground().setTintList(null);
    }
    ...
    public void setIconLock(boolean locked) {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(
                locked ? R.drawable.ic_lock_24dp : R.drawable.ic_lock_open_24dp, 0, 0, 0);
    }
